#!/bin/bash

set -euo pipefail

export SERVER_FULLNAME="${1:-dev.db.faims.edu.au}" # dev, alpha, testing, selenium
cp burial.json faims3-temp-notebook-migrator/
cd faims3-temp-notebook-migrator
echo "Load Burial ${SERVER_FULLNAME}"
#sed -e "s/##SPID##/${SPID}/g" 20220110-model1.json > fiptemp.json
bash loadNotebook.sh $SERVER_FULLNAME "Burial Mounds Monitoring $(date)" "burial.json" "burial"
rm -f faims3-temp-notebook-migrator/burial.json
rm -f faims3-temp-notebook-migrator/testnotebook.json